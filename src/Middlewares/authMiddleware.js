  ...
  try {
    ...
    if (req.body.userId)
      if (!mongoose.isValidObjectId(req.body.userId))
        return res
          .status(400)
          .send({ status: false, msg: "userId of req body is invalid" });
    if (req.params.bookId)
      if (!mongoose.isValidObjectId(req.params.bookId))
        return res
          .status(400)
          .send({ status: false, msg: "bookId of path params is invalid" });

    if (req.body.userId)
      if (req.decoded.userId != req.body.userId)
        return res
          .status(400)
          .send({ status: false, msg: "Your userId and token is not matched" });

    let findBook;
    if (req.params.bookId)
      findBook = await bookModel.findById(req.params.bookId);
    if (req.params.bookId)
      if (!findBook)
        return res
          .status(404)
          .send({ status: false, msg: "Book not found with given bookId" });
    if (req.params.bookId)
      if (req.decoded.userId != findBook.userId)
        return res
          .status(400)
          .send({
            status: false,
            msg: "Can not Update or Delete a book with provided bookId",
          });
    next();
  } catch (error) {
