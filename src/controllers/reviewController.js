  ...
  try {
    ...
    if (data.bookId != req.params.bookId)
      return res
        ...
        .send({
          status: false,
          msg: "bookId of path params and req body should be the same",
        });
    ...
    if (!data.reviewedAt)
      return res
        .status(400)
        .send({ status: false, msg: "reviewedAt is mandatory" });
    if (!data.rating)
      return res
        .status(400)
        .send({ status: false, msg: "rating is mandatory" });
    if (data.rating >= 5.1 || data.rating < 1)
      return res
        .status(400)
        .send({ status: false, msg: "please rate in between 1 to 5" });
    data.rating = Math.round(data.rating);
    if (data.review == "")
      return res
        .status(400)
        .send({ status: false, msg: "review field can not be empty" });

    let dateFormat = /^(19|20)\d{2}\-(0[1-9]|1[0-2])\-(0[1-9]|1\d|2\d|3[01])$/;
    if (!dateFormat.test(data.reviewedAt.trim()))
      return res
        .status(400)
    ...
    let findBook = await bookModel.findOneAndUpdate(
      { _id: data.bookId, isDeleted: false },
      { $inc: { reviews: 1 } },
      { new: true }
    );
    if (!findBook)
    ...
    return res
      ...
      .send({
        status: true,
        message: "Success",
        data: { _id, bookId, reviewedBy, reviewedAt, rating, review },
      });
  } catch (error) {
...
const updateReview = async function (req, res) {
  try {
    ...
    if (
      !mongoose.isValidObjectId(req.params.bookId) ||
      !mongoose.isValidObjectId(req.params.reviewId)
    )
      return res
        .status(400)
        .send({ status: false, msg: "bookId or reviewId is invalid" });

    if (data.review == "" || data.reviewedBy == "")
      return res
        .status(400)
        .send({ status: false, msg: "Please put value for field" });
    ...
    let findReview = await reviewModel
      .findOneAndUpdate(
...
const deleteReview = async function (req, res) {
  try {
    ...
    let decBookReview = await bookModel.findOneAndUpdate(
