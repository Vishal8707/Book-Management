  ...
  try {
    ...
    if (!validName.test(data.name))
      return res
        ...
        .send({
          status: false,
          Msg: "User Name can not contain numerical values or special characters",
        });
    if (!validNumber.test(data.phone))
      return res
        .status(400)
        .send({
          status: false,
          Msg: "Phone Number should only contain 10 digits in it and should start with 6/7/8/9",
        });
    if (!emailRegex.test(data.email))
      return res
        .status(400)
        .send({ status: false, Msg: "email format is Invalid" });
    if (!passwordRegex.test(data.password))
      return res
        .status(400)
        .send({
          status: false,
          Msg: "Password should contain at least 8 and max 15 characters with 1 upper, lower case and special char",
        });

    let findEmailPhone = await userModel.findOne({
    ...
    if (address || address == "")
      if (!address.street || !address.city || !address.pincode)
        return res
          ...
          .send({
            status: false,
            msg: "address should contain 'street','city','pincode'",
          });
...
const login = async function (req, res) {
  try {
    ...
    if (Object.keys(data).length == 0)
      return res
        .status(400)
        .send({ status: false, msg: "request body cant be empty" });
    ...
    if (Object.keys(data).length > 2)
      return res
        ...
        .send({
          status: false,
          msg: "request body can only contain email and password",
        });
    ...
    }; //,iat: Math.floor(Date.now() / 1000),exp: Math.floor(Date.now() / 1000) + (30 * 60)
