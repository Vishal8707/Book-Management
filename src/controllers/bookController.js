...
const book = async function (req, res) {
  try {
    ...
    if (!excerpt)
      return res
        .status(400)
        .send({ status: false, msg: "excerpt is mandatory" });
    if (!userId)
      return res
        .status(400)
    ...
    if (!releasedAt)
      return res
        .status(400)
        .send({ status: false, msg: "releasedAt is mandatory" });

    const titleRegex = /^[a-z A-Z_]{3,20}$/;
    if (
      !titleRegex.test(title) ||
      !titleRegex.test(category) ||
      !titleRegex.test(subcategory)
    )
      return res
        .status(400)
        .send({
          status: false,
          Msg: "'title', 'category', 'subcategory' can not contain numerical values, special characters or empty spaces",
        });
    ...
    if (!validUser)
      return res
        ...
        .send({
          status: false,
          msg: "User not found with the provided UserID",
        });

    const isbnRegex = /^(?=(?:\D*\d){13}(?:(?:\D*\d){3})?$)[\d-]+$/g;
    if (!isbnRegex.test(data.ISBN.trim()))
      return res
        .status(400)
        .send({ status: false, msg: "ISBN number format is incorrect" });
    ...
    if (findISBN)
      return res
        .status(400)
        .send({ status: false, msg: "ISBN number already exists" });
    if (findTitle)
      return res
        .status(400)
    ...
    if (!data.reviews == 0)
      return res
        ...
        .send({
          status: false,
          msg: "Review count can not be greater or lesser than 0 at the time of creation of book",
        });
    if (!dateFormat.test(releasedAt.trim()))
      return res
        .status(400)
        .send({ status: false, msg: "Date format is wrong" });
...
const getBooks = async function (req, res) {
  try {
    let query = req.query;
    let { userId, category, subcategory, title, excerpt, ISBN, reviews } =
      query;
    if (userId == "" || category == "" || subcategory == "")
      return res
        .status(400)
        .send({ status: false, msg: "query params value can't be empty" });
    if (userId)
      if (!mongoose.isValidObjectId(userId))
        return res
          .status(400)
    ...
    if (title || excerpt || ISBN || reviews)
      return res
        .status(400)
        .send({
          status: false,
          msg: "You can only fetch books by its userId,category,subcategory",
        });
...
const getBooksById = async function (req, res) {
  try {
    ...
    let reviewsList = await reviewModel
...
const updateBook = async function (req, res) {
  try {
    ...
    let exist = await bookModel.findOne({
    ...
    if (exist)
      return res
        ...
        .send({
          status: false,
          msg: "Can not update unique fields which are already exist",
        });
    let finalData = await bookModel.findOneAndUpdate(
    ...
    if (!finalData)
      return res
        .status(404)
        .send({ status: false, msg: "Document not found for update" });
...
const deleteBookById = async function (req, res) {
  try {
    let bookId = req.params.bookId;
    let deleteDoc = await bookModel.findOneAndUpdate(
    ...
    return res
      .status(200)
      .send({ status: true, Info: "Document deleted successfully" });
  } catch (error) {
    return res.status(500).send({ errorMsg: error.message });
  }
};
